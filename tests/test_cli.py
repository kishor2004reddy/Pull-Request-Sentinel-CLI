...
def test_resolve_pat_prefers_env(monkeypatch):
    ...
    monkeypatch.setattr(cli, "getpass", lambda *a, **k: pytest.fail("should not prompt"))
    assert cli._resolve_azure_pat("scope") == "env-token"


def test_resolve_pat_prompts_when_tty_and_no_env(monkeypatch):
    monkeypatch.setattr(cli.sys, "stdin", _FakeStdin(True))
    monkeypatch.setattr(cli, "getpass", lambda *a, **k: "  typed-token  ")
...
def test_resolve_pat_errors_when_not_a_tty(monkeypatch):
    ...
    monkeypatch.setattr(cli, "getpass", lambda *a, **k: pytest.fail("must not prompt in CI"))
...
def test_resolve_pat_errors_when_prompt_empty(monkeypatch):
    monkeypatch.setattr(cli.sys, "stdin", _FakeStdin(True))
    monkeypatch.setattr(cli, "getpass", lambda *a, **k: "   ")
    with pytest.raises(click.UsageError):
        cli._resolve_azure_pat("scope")
